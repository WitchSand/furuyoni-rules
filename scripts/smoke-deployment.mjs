...
async function fetchText(path, expectedStatus = 200) {
  ...
  const response = await fetch(url, { redirect: "follow", headers: { "user-agent": "furuyoni-release-smoke/1.0" } });
...
const pages = await Promise.all(routes.map((route) => fetchText(route)));
...
const wrongBaseResponse = await fetch(wrongBase, { redirect: "manual" });
